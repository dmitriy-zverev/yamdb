...
class TitleSerializer(serializers.ModelSerializer):
    ...
    review = serializers.StringRelatedField(read_only=True, many=True)
    ...
    def to_representation(self, instance):
        ...
        data['rating'] = calculate_title_rating(
            ReviewSerializer(instance.reviews.all(), many=True).data)
...
class ReviewSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if self.instance is None and text is None:
            raise serializers.ValidationError(
                {'text': 'Неверный формат текста'})

        if self.instance is None and score is None:
            raise serializers.ValidationError(
                {'score': 'Неверный формат рейтинга'})

        if score is not None and (score < 1 or score > 10):
            raise serializers.ValidationError(
                {'score': 'Рейтинг может быть от 1 до 10'})
